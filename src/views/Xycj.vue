...
export default {
  ...
  data () {
    return {
      headerList: "排序,班级,姓名,学号,平均成绩,平均绩点", // 注意：逗号是英文的逗号
      bodyInfoList: []
    }
  },
  mounted () {
    this.axios.post("http://ecampus.nfu.edu.cn:2929/jw-amsi/AmsJxbXsZgcj/listXsjd?jwloginToken=" + this.$store.state.token + "&pageSize=65536&sznj=" + this.$store.state.sznj + "&yxId=" + this.$store.state.yxId + "&pg=1&"
    ).then((result) => {
